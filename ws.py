...
app = FastAPI(title="WAN 2.1 WebSocket API")
...
async def websocket_endpoint(websocket: WebSocket):
    """Handles real-time WebSocket-based video generation requests."""
...
# ========== WebSocket Streaming Video Download ==========
@app.websocket("/ws/download")
async def websocket_stream_video(websocket: WebSocket):
    """Streams video data to the WebSocket client in real-time."""
    await websocket.accept()
    try:
        data = await websocket.receive_json()
        video_path = data.get("video_path")

        file_path = os.path.join(os.getcwd(), video_path)
        ...
        logging.info("WebSocket client disconnected during streaming.")
    except Exception as e:
        await websocket.send_json({"error": str(e)})
